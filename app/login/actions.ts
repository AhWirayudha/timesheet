...
export const signUp = validatedAction(signUpSchema, async (data, formData) => {
  ...
  if (inviteId) {
    ...
    // They can only create personal timesheets until they join a team
    await logActivity(null, createdUser.id, ActivityType.SIGN_UP);
    
    // Set session and redirect without creating team
    await setSession(createdUser);
    redirect('/dashboard');
  }

  // Only create team member if there's a team (invitation case)
  if (teamId && userRole) {
...
export const inviteTeamMember = validatedActionWithUser(
  ...
  async (data, _, user) => {
    ...
    if (userTeamRole.length === 0) {
      return { error: 'User team role not found' };
    }

    // Check if user is an admin (owner) to send invitations
    if (userTeamRole[0].role !== 'owner') {
      return { error: 'Only team owners can send invitations' };
    }
    ...
    return { success: 'Invitation sent successfully. The user will see it in their invitations page.' };
  }
);
...
export const createTeam = validatedActionWithUser(
  ...
  async (data, _, user) => {
    ...
    if (existingTeam.length > 0) {
      console.log('User already part of team:', existingTeam[0]);
      return { error: 'You are already part of a team' };
    }
    ...
    try {
      ...
      if (!newTeam) {
        console.error('Failed to create team - no team returned');
      ...
      await logActivity(newTeam.id, user.id, ActivityType.CREATE_TEAM);
